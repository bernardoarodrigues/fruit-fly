...
def main():
    ...
    for ai,(clock,proxy) in enumerate((c,p) for c in plan['clocks'] for p in plan['input_proxies']):
        ...
        for (panel,rank),pair in sorted(pairs.items()):
            ...
            factor=float(raw[dt<=12].max());assert factor>0
            driver=raw/factor
        ...
        arms.append(dict(clock=clock,input_proxy=proxy,models=models,profile=profile,summaries=summaries,cases=cases))
    arrays['time_s']=query
    ...
    result=dict(status='complete_declared_reporter_filter_comparison',completed_utc=datetime.now(timezone.utc).isoformat(),plan=pin(PLAN),arrays=pin(ARRAYS),
                arms=arms,physiological_parameter_selected=False,neural_run=False,runtime_changed=False)
