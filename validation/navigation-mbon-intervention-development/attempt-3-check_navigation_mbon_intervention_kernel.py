...
def main():
    ...
    try:
        ...
        decayed_h = float(original_out["selected_h"][21, 2]) * math.exp(-.1 / 5.)
        observations["negative_arrival_states"] = {"decayed_h": decayed_h,
            "original_h": float(original_out["selected_h"][22, 2]),
            "intervened_h": float(altered_out["selected_h"][22, 2]),
            "difference": float(original_out["selected_h"][22, 2] - altered_out["selected_h"][22, 2])}
        ck("negative_original_ordered_delivery_exact", original_out["selected_h"][22, 2] == (decayed_h + 2.) + 1.)
        ck("negative_intervened_ordered_delivery_exact", altered_out["selected_h"][22, 2] == decayed_h + 1.)
        ...
        decayed_s = float(original_out["selected_s"][21, 3]) * math.exp(-.1 / 5.)
        ck("positive_original_ordered_delivery_exact", original_out["selected_s"][22, 3] == decayed_s + 9.)
        ck("positive_intervened_ordered_delivery_exact", positive_out["selected_s"][22, 3] == decayed_s)
        ...
        # An explicit post-restore trace expansion affects only observations.
        # Keep original event indices explicit because the default follows the
        # selected set by design in the frozen panel wrapper.
        ids, ptr, targets, weights, inputs, _ = fixture()
        narrow = base.FactorialNetwork(ids, ptr, targets, weights, "H1", inputs, [0, 2], 11)
        narrow.advance(u[:20], p)
        narrow_cp = narrow.checkpoint()
        unchanged, expanded = restore(narrow_cp), restore(narrow_cp)
        expanded.replace_selected_indices([0, 2, 3, 4, 5])
        def dynamics(cp_value):
            return {key: value for key, value in cp_value.items() if key != "selected_indices"}
        same("observation_expansion_all_dynamical_state_unchanged", dynamics(narrow_cp), dynamics(expanded.checkpoint()))
        same("observation_expansion_order", expanded.selected_indices, np.array([0, 2, 3, 4, 5], np.int32))
        same("observation_expansion_reverse_columns", expanded._selected_column, np.array([0, -1, 1, 2, 3, 4], np.int32))
        narrow_out = unchanged.advance(u[20:], p, log_selected_events=True, event_indices=[0, 2])
        expanded_out = expanded.advance(u[20:], p, log_selected_events=True, event_indices=[0, 2])
        same("observation_expansion_endpoint_dynamics", dynamics(unchanged.checkpoint()), dynamics(expanded.checkpoint()))
        for key in narrow_out:
            if key.startswith("selected_") and key != "selected_events":
                same("observation_expansion_original_columns_" + key, narrow_out[key], expanded_out[key][:, :2])
            else:
                same("observation_expansion_unchanged_output_" + key, narrow_out[key], expanded_out[key])
        for name, bad_indices in (("duplicate", [0, 0]), ("range", [6]), ("fraction", [1.5])):
            before_selection = expanded.checkpoint()
            rejects("reject_observation_" + name, lambda: expanded.replace_selected_indices(bad_indices))
            same("reject_observation_" + name + "_nonmutation", before_selection, expanded.checkpoint())
