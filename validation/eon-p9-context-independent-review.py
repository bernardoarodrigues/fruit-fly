...
for seed in plan['seeds']:
 for cond in plan['condition_order']:
  ...
  if cond.startswith('p9_'):check(key+':50_500ms_P9_context_forward_silent',not late.any())
  ...
  all_inputs[(seed,cond)]={'uniforms':inp['uniforms'],'candidate':candidate,'applied':applied,'spikes':(si,ticks),'final':fin}
...
comparisons=[]
for seed in plan['seeds']:
 a=all_inputs[(seed,'p9_only')];b=all_inputs[(seed,'p9_plus_taste_taste_outputs_blocked')]
 ka=~np.isin(a['spikes'][0],sources[2:]);kb=~np.isin(b['spikes'][0],sources[2:])
 for field,x,y in zip(['indices','ticks'],a['spikes'],b['spikes']):same(f'{seed}:taste_output_block_restores_all_non_taste_spike_'+field,x[ka],y[kb])
 target=np.ones(n,bool);target[sources[2:]]=False
 for field in ['voltage_mv','synaptic_mv','last_spike_tick']:
  same(f'{seed}:taste_output_block_restores_non_taste_endpoint_'+field,a['final'][field][target],b['final'][field][target])
 a=all_inputs[(seed,'no_events')];b=all_inputs[(seed,'p9_only_p9_outputs_blocked')]
 target=np.ones(n,bool);target[sources[:2]]=False
 for field in ['voltage_mv','synaptic_mv','last_spike_tick']:
  same(f'{seed}:P9_output_block_preserves_non_P9_rest_endpoint_'+field,a['final'][field][target],b['final'][field][target])
