...
## First male transfer test, executed after this audit

The [P9-context panel](../docs/eon-p9-context-assay.md) now tests the known male P9 pair and eleven LgAG2 cells in 12 fixed 0.5 s trials: four stimulation combinations plus two output-block controls, for seeds 11 and 12. It preserves the full male graph, model and weights. The unknown ascending pair is omitted, and the input cohort, shorter duration and two seeds differ from the source protocol. No body ran.

All **167 consistency checks** pass and all **606,759 ordered spikes** are retained. P9 recruits DNa02 activity, and taste changes the response. However, both oDN1 cells remain silent during **50–500 ms** in every P9-only and P9-plus-taste trial. Taste coactivation changes left DNa02 in opposite directions across the two seeds; one combined trial also recruits MN9. The lowest sampled voltage is **−474.39 mV**. Blocking P9 output removes downstream activity in the P9-only control, while blocking taste output restores the P9-only motor readout counts.

This supports testing context-dependent sensory modulation, but does not reproduce a reliable forward-walking signal or resolve the model's physiological failure. A source-graph control remains a possible diagnostic; **the user's subsequent steering makes the inhibitory factorial the primary next work and explicitly skips the proposed embodied benchmark**. Body gain tuning cannot by itself explain a missing sustained neural output. No global gain change, body integration or further Eon trial was performed after this fixed panel. H1's [promotion criteria](../docs/inhibitory-promotion-gates.md) require physiological, stimulus-contrast, seed and recurrent evidence in addition to numerical bounds.
