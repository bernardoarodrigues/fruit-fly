...
**Design advice only; no recorded-input factorial experiment has yet run.** This proposal was initially paused for the Eon comparison. The user's 2026-09-05 steering now makes it the primary scientific focus after independent review/commit of the bounded Eon neural results, and explicitly skips the proposed body integration benchmark. The [promotion gates](inhibitory-promotion-gates.md) require physiological and recurrent evidence in addition to numerical bounds. This note changes no runtime, parameters, source recordings, results or previously frozen plans.

The starting evidence is the [381-check independent replay](negative-voltage-replay-independent-review.md), its [producer](../scripts/negative-voltage-replay.py), and the two separate kernels in [neural.py](../fruitfly/neural.py) and [conductance.py](../fruitfly/conductance.py). The replay exactly reconstructs the signed-current model under recorded inputs. It does not validate the extreme negative model voltages biologically. Switching directly to the existing full conductance backend changes excitation and refractory handling as well as inhibition; that comparison cannot isolate the inhibitory driving-force change.
