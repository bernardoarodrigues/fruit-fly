...
The [MBON input audit](navigation-ladder-mbon-inputs.md) is now complete: all six reconstructed synaptic histories match independently, and Kenyon cells supply about 92.15% of positive accepted increments over the declared interval. This satisfies the predeclared gate for one [KC-to-MBON delivery intervention](navigation-mbon-intervention.md). Its isolated implementation passes synthetic checks and a six-condition actual-graph inactive prefix check. The next active batch suppresses only the selected deliveries for 0.5–1.5 s, records MBON voltage/p/h, and uses all three seeds with matched EA/constant controls. Analyze the complete batch together before any new decision or promotion.
