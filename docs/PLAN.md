...
The next [KC/PN/APL inventory](pn-kc-apl-inventory.md) is also complete and independently reviewed across twelve saved histories. It finds almost universal KC activity, dominant positive KC-to-KC increments, generic fast dopamine input comparable in magnitude to ALPN input, and near-ceiling model APL events. These are distinct calibration problems, not a reason for another global gain sweep. Proceed with the [source-defined single-event KC EPSP/EPSC compatibility calculation](../research/21-pn-kc-physiology-calibration.md), preserving the separate intrinsic current-response constraint and excluding threshold/APL fitting. The [APL source review](../research/22-apl-local-feedback-constraints.md) and [spatial-data availability](../research/23-kc-contact-location-availability.md) inform later mechanism design. The frozen [PN→KC/APL plan](pn-kc-apl-calibration-plan.md) remains the prior methodological record; source-specific criteria and untouched evaluation data must be fixed before future fitting.
