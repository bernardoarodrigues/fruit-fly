...
The [MBON input audit](navigation-ladder-mbon-inputs.md) and its selected [KC-to-MBON delivery intervention](navigation-mbon-intervention-results.md) are complete. All six branches and independent delivery/state, numerical and outcome reviews pass. The causal effect is large but physiologically inadequate: MBON12/14 become silent, MBON13 remains highly active, matched odor contrast is negligible, and saturation returns after restoration. Whole-network persistence is not resolved. The completion trigger is paused; do not restart the batch. Proceed to the bounded read-only inventory in the [PN→KC/APL calibration plan](pn-kc-apl-calibration-plan.md), then freeze a local assay with source-compatible measurements and held-out conditions. The [reviewed source note](../research/20-kc-mbon-functional-calibration.md) supplies constraints, not interchangeable hard tolerances or a universal 5% recruitment target.
