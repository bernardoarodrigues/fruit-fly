...
Airborne odor advection uses the configured wind. A FlyBody head-frame airflow receptor is disabled until its anatomical basis is independently verified. Source perspective eye cameras are not FlyGym ommatidia. Render lighting uses the native FlyBody lights; the light control scales original world diffuse/ambient/specular and headlight coefficients. Snapshot illumination reports `source_native_flybody`, actual coefficients and the relative gain.

## Observer and neural boundary

`observe()` is cached and supplies only the existing runner's pose, bilateral odor, taste booleans/maps, physiology, motor state, light and limited proprioceptive fields, plus truthfully disabled wind/grooming/vision status. The neural encoders consume their defined subsets. `snapshot()` separately includes resource inventories, configuration, source provenance, capability flags, illumination and compact physical diagnostics.
...
An opt-in [rolling-reference extension](flybody-rolling-runtime.md) now passes separate body/RPC comparisons through twelve seconds. It uses explicit `reference_mode: "rolling"` and `horizon_s: null`; the bounded configuration and its historical results remain unchanged. Longer full-neural/browser validation is tracked separately.
