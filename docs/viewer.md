...
The [rolling viewer inspection](../validation/flybody-rolling-viewer/inspection.json) verifies operation beyond that bounded horizon, pause at 3.73 s, camera invariance and reset. The later [airflow viewer inspection](../validation/flybody-airflow-viewer/inspection.json), on port 8773 with `configs/male-flybody-rolling-airflow-probe.json`, reaches 5.44 s. Moving antennae produce changing local flow; the display names the proximal sampling point and missing distal-receptor/neural mapping. Paused camera changes preserve airflow and other recorded state, and reset restores initial values exactly. Its source floor remains unbounded and its motor-probe assay is artificial stimulation.
