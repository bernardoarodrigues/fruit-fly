...
The released [FlyBody walking policy](flybody-inference-trial.md) now loads in a separately pinned Apple Silicon CPU environment and completes a short stable source-body walk. All observation/action dimensions match the original checkpoint; an independent NumPy mean path agrees within the declared tolerance. A longer fixed motor comparison is pending before any integration or default change.
