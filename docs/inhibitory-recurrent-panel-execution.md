...
**Completed:** all 60 trials finished on 2026-09-05 at 10:58:42 UTC, with no trial failure, resource limit or replacement. All independent saved-data audits pass. See the [combined results and promotion decision](inhibitory-recurrent-panel-results.md); the progress snapshots below retain their original scope and are superseded by that report. H1 remains experimental, and the completion heartbeat has been paused.
...
### All C0 controls admitted before altered arms

All fifteen C0 trials subsequently completed and passed both the original-prefix gates and independent saved-data review. The parent published its fifteen-gate receipt and started C1 on 2026-09-05 at 06:22:31 UTC; the [admission record](../validation/inhibitory-recurrent-panel-c0-admission.json) pins every real gate and confirms all C0 terminal timestamps precede that start. [Complete C0 results](inhibitory-recurrent-panel-c0-results.md) now supersede the first-seed snapshot for three-seed interpretation: 21,546,618 spikes, consistent first-hop odor contrasts, seed-dependent whole-network contrasts, continued off-period activity and silent forward readouts. Exact boundary source spikes remain distinguished from zero off-period external events.

The [separate scalar audit](inhibitory-panel-reference-audit.md) passes all 125 retained cases with an independent integral/ODE implementation. The [H saved-data reader](inhibitory-recurrent-panel-H-saved-data-audit.md) adds fixed-reference checks to state/event/count reconstruction and passes its 30-check preflight. The first C1, H0 and H1 no-input controls also completed their independent audits. C1 passes 37,624 checks, and H0/H1 each pass 53,827. All remain at rest with zero spikes; each H audit verifies 1,800 reference rows representing one unique rest-state tuple. These quiet controls alone do not establish stimulated or nonzero-conductance H-network dynamics. The parent subsequently completed all remaining trials; their [batch reviews](inhibitory-recurrent-panel-batch-integrity-review.md) and [combined analysis](inhibitory-recurrent-panel-results.md) supply the completed evidence.
