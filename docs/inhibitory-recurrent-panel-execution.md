...
The [separate scalar audit](inhibitory-panel-reference-audit.md) passes all 125 retained cases with an independent integral/ODE implementation. The [H saved-data reader](inhibitory-recurrent-panel-H-saved-data-audit.md) adds fixed-reference checks to state/event/count reconstruction and passes its 30-check preflight. The first C1, H0 and H1 no-input controls have also completed their independent audits. C1 passes 37,624 checks, and H0/H1 each pass 53,827. All remain at rest with zero spikes; each H audit verifies 1,800 reference rows representing one unique rest-state tuple. These quiet controls do not establish stimulated or nonzero-conductance H-network dynamics. The same frozen parent continues the remaining trials.
