...
def main():
    ...
    result['prior_attempt']=dict(receipt='validation/navigation-ladder-anatomy-initial-v0.json',receipt_sha256=sha(ROOT/'validation/navigation-ladder-anatomy-initial-v0.json'),source='validation/navigation-ladder-anatomy-initial-v0-source.py',source_sha256=sha(ROOT/'validation/navigation-ladder-anatomy-initial-v0-source.py'),reason='Initial full cohorts and contact counts were correct, but convenience side partitions omitted10midline(M)DNs. Final addsM and exhaustive partition checks; original1968check receipt remains retained.')
    ...
    try:
        ...
        def partition(indices,series):
            return {key:indices[(series.iloc[indices].isna() if key=='missing' else series.iloc[indices].eq(key)).to_numpy()].tolist() for key in ['L','R','M','unknown','missing']}
        ...
        for name,group in result['groups'].items():
            for field in ['root_side_indices','soma_side_indices','current_select_side_indices']:
                flattened=[i for bucket in group[field].values() for i in bucket]
                ck('exhaustive_disjoint_side_partition',sorted(flattened)==group['indices'] and len(set(flattened))==len(flattened),[name,field])
