...
def main(expected_results_sha):
    ...
    initial = ROOT / 'validation/kc-synaptic-response-plot-initial'
    if initial.exists():
        report['visual_correction'] = dict(
            reason='Initial footer overlapped bottom Time (ms) labels. Footer moved from y=.09 to y=.035; numerical inputs and plotted quantities unchanged.',
            initial_artifacts=[record(initial / name) for name in [
                'plot_kc_synaptic_response.py', 'kc-synaptic-response-figure.png',
                'kc-synaptic-response-plot-receipt.json']])
