...
def make_plan():
    return dict(created_at=datetime.now(timezone.utc).isoformat(),
        ...
        amendment={'previous_plan': 'validation/flybody-airflow-hash-v0-plan.json',
                   'previous_result': 'validation/flybody-airflow-hash-v0-result.json',
                   'reason': 'Exclude fresh owning Python arrays from native-state hashes. With constraint-island discovery disabled, unallocated island fields return owning arrays with no native backing; their undefined contents changed while all actual physical state remained identical. No geometry probe, numerical tolerance or model change.'},
...
def numeric_arrays(owner):
    ...
    for name in dir(owner):
        ...
        try:
            ...
            continue
        # Some unallocated MuJoCo pointers are exposed as fresh owning NumPy
        # arrays. They are not views of native state and contain undefined
        # allocation contents. Never inspect/hash their values.
        if isinstance(value, np.ndarray) and value.dtype.kind in 'biuf' and not value.flags.owndata:
...
def run():
    ...
    for name in dir(live):
        if name.startswith('_'):
            continue
        try:
            first = getattr(live, name)
        except Exception:
            continue
        if isinstance(first, np.ndarray) and first.dtype.kind in 'biuf' and first.flags.owndata:
            second = getattr(live, name)
            unbacked.append(dict(name=name, shape=list(first.shape), dtype=str(first.dtype),
                owns_data=True, base_is_none=first.base is None,
                repeat_pointer_same=first.ctypes.data == second.ctypes.data,
                raw_values_inspected=False))
    ...
    report = dict(complete=True, passed=all(c['passed'] for c in checks), plan_sha256=digest(PLAN),
        ...
        excluded_python_owned_arrays=unbacked,
        constraint_islands=dict(enableflags=int(m.opt.enableflags), nisland=int(live.nisland),
            enabled=bool(m.opt.enableflags & int(mujoco.mjtEnableBit.mjENBL_ISLAND))),
