#!/usr/bin/env python3
"""Experiment-local panel tracing and interval capture; frozen kernels unchanged.
...
REFERENCE_NAMES = ("max_h", "min_threshold_margin", "max_log_p_over_h")
REFERENCE_COLUMNS = ("before_v_mv", "before_p_mv", "before_h", "dt_ms",
    "prethreshold_v_mv", "threshold_margin_mv", "stiffness", "tail_bound_mv",
    "inverse_residual", "inverse_iterations", "cutoff", "log_p_over_h")


@njit(cache=True)
def _reference_candidate(tick, cell, old_v, old_p, old_h, scratch, found, ticks, cells, values):
    # Eligible intervals have completed the unchanged H solver. Refractory
    # intervals and h=0 ratios are excluded from their respective rankings.
    after = scratch[cell, 0]
    margin = abs(after + 45.)
    stiffness = (1. + old_h) * .1 / 20.
    log_ratio = np.nan
    if old_h > 0.:
        log_ratio = np.log(old_p) - np.log(old_h) if old_p > 0. else -np.inf
    for rank in range(3):
        eligible = rank != 2 or old_h > 0.
        better = False
        if eligible:
            if not found[rank]:
                better = True
            elif rank == 0:
                better = old_h > values[rank, 2]
            elif rank == 1:
                better = margin < values[rank, 5]
            else:
                better = log_ratio > values[rank, 11]
        if better:
            found[rank] = True
            ticks[rank], cells[rank] = tick, cell
            values[rank, 0], values[rank, 1], values[rank, 2] = old_v, old_p, old_h
            values[rank, 3], values[rank, 4], values[rank, 5] = .1, after, margin
            values[rank, 6] = stiffness
            values[rank, 7], values[rank, 8] = scratch[cell, 3], scratch[cell, 4]
            values[rank, 9], values[rank, 10], values[rank, 11] = scratch[cell, 5], scratch[cell, 6], log_ratio


@njit(cache=True)
def _merge_references(found, ticks, cells, values, tick_found, tick_ticks, tick_cells, tick_values):
    # Called only after the entire macro tick completes. Strict comparisons keep
    # the earlier tick, and within a tick _reference_candidate kept lowest index.
    for rank in range(3):
        if tick_found[rank]:
            better = not found[rank]
            if not better:
                if rank == 0:
                    better = tick_values[rank, 2] > values[rank, 2]
                elif rank == 1:
                    better = tick_values[rank, 5] < values[rank, 5]
                else:
                    better = tick_values[rank, 11] > values[rank, 11]
            if better:
                found[rank] = True
                ticks[rank], cells[rank] = tick_ticks[rank], tick_cells[rank]
                for j in range(values.shape[1]):values[rank, j] = tick_values[rank, j]
...
def _run(first_tick, uniforms, probabilities, hybrid, package, a, b, coefficient,
         ...
         indptr, targets, weights, inputs, selected, selected_column, log_events, event_mask):
    ...
    ref_found = np.zeros(3, np.bool_)
    ref_ticks = np.full(3, -1, np.int64)
    ref_cells = np.full(3, -1, np.int32)
    ref_values = np.full((3, 12), np.nan)
    tick_ref_found = np.zeros(3, np.bool_)
    tick_ref_ticks = np.full(3, -1, np.int64)
    tick_ref_cells = np.full(3, -1, np.int32)
    tick_ref_values = np.full((3, 12), np.nan)
    ...
    for k in range(nsteps):
        ...
        tick_ref_found[:] = False
        tick_ref_ticks[:] = -1
        tick_ref_cells[:] = -1
        tick_ref_values[:, :] = np.nan
        ...
        for cell in range(ncells):
            ...
            old_v, old_s, old_h = v[cell], s[cell], h[cell]
            ...
            if available:
                if hybrid:
                    ...
                    _reference_candidate(tick, cell, old_v, old_s, old_h, scratch,
            ...
            for edge in range(indptr[source], indptr[source + 1]):
                ...
                if blocked[source]:
                    ...
                    if package == 1 or active[target]:
                        ...
                        disposition = 1
                if log_events and event_mask[target]:
        ...
        _merge_references(ref_found, ref_ticks, ref_cells, ref_values,
                          tick_ref_found, tick_ref_ticks, tick_ref_cells, tick_ref_values)
        completed += 1
...
class FactorialNetwork:
    ...
    def advance(self, uniforms, probabilities, *, log_selected_events=False, event_indices=None):
        ...
        event_indices = self.selected_indices if event_indices is None else self._indices(event_indices)
        if not np.isin(event_indices, self.selected_indices).all():
            raise ValueError("Event indices must be a subset of the fixed selected traces")
        event_mask = np.zeros(self.n_neurons, np.bool_)
        ...
        values = _run(start,u,p,self.arm.startswith("H"),int(self.arm[1]),self._a,self._b,self._coefficient,
            ...
            self.indptr,self.targets,self.weights,self.input_indices,self.selected_indices,self._selected_column,bool(log_selected_events),event_mask)
        (done,code,cell,phase,before,ii,tt,candidate,applied,sv,ss,sh,spre,savail,sfired,sactive,
         vmin,vmax,imin,imax,nf,below,invalid,work,solver,edges,progress,events,
        ...
        result["schema"]["event_graph_indices"] = event_indices.tolist()
        result["schema"]["reference_names"] = REFERENCE_NAMES
        result["schema"]["reference_columns"] = REFERENCE_COLUMNS
        result["reference_intervals"] = {
            "found":rf,"ticks":rt,"graph_indices":ri,"values":rv,
            "scope":"Completed macro ticks in this advance only; H available intervals before threshold/reset; ties earliest tick then lowest graph index",
            "max_h_also_ranks":"Largest stiffness, since dt and membrane tau are fixed",
            "ratio_domain":"h>0; p=0 is eligible with log_p_over_h=-inf; h=0 is ineligible",
            "partial":None}
        if code:
            result["reference_intervals"]["partial"] = {
                "found":tf,"ticks":ttick,"graph_indices":ti,"values":tv,
                "scope":"Successfully integrated available H intervals reached by the serial commit scan in the attempted failed tick only; not completed macro states"}
        if code:
    ...
    def step(self, uniforms, probabilities, *, log_selected_events=False, event_indices=None):
        return self.advance(np.asarray(uniforms).reshape(1,-1), probabilities, log_selected_events=log_selected_events, event_indices=event_indices)
