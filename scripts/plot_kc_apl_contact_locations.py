...
def main(expected_results_sha):
    ...
    for which, left, title in [(0, .205, 'A  KC targets: 4,064 cells'), (1, .64, 'B  APL targets: 2 cells')]:
        ax = fig.add_axes([left, .53, .32, .325])
    ...
    cax = fig.add_axes([.968, .53, .007, .325])
    ...
    initial = ROOT / 'validation/kc-apl-contact-location-plot-initial'
    if initial.exists():
        report['visual_correction'] = dict(reason='Initial colorbar100 label clipped at canvas edge. Heatmaps narrowed by .01 figure width; colorbar moved inward. Data, percentages and samples unchanged.',
            initial_artifacts=[record(initial / name) for name in [
                'plot_kc_apl_contact_locations.py', 'kc-apl-contact-location-figure.png',
                'kc-apl-contact-location-plot-receipt.json']])
