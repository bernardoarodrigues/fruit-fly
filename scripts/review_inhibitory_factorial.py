...
def run():
    initial = json_read(ROOT / "validation/inhibitory-factorial-independent-review-initial.json")
    record(ROOT / "validation/inhibitory-factorial-independent-review-initial.py")
    R["prior_review_attempt"] = {"receipt": "validation/inhibitory-factorial-independent-review-initial.json",
        "script": "validation/inhibitory-factorial-independent-review-initial.py", "checks_completed": initial["check_count"],
        "error": initial["errors"], "cause": "Reviewer used pass instead of passed for the prior input-audit schema. Stopped before inspecting trial arrays; no producer failure or data changes. Corrected the schema key only."}
    ...
    check("prior_input_audit_passed", audit["passed"] and all(c["passed"] for c in audit["checks"]))
    cells = {}
