...
r=json.loads((ROOT/'validation/apl-recruitment-results.json').read_text());p=json.loads((ROOT/'validation/apl-recruitment-plan.json').read_text());a=np.load(ROOT/r['arrays']['path']);t=a['time_s']
...
for i,c in enumerate(p['cells']):
    ...
    for side in [0,1]:
        ...
        for f in r['fits']:
            color='#087f8c' if f['separate_kinetics'] else '#d97706'
...
fig.suptitle('APL recruitment and removal kinetics: complete comparison\nAll 11 cells, 65 repeats, four passive variants and two voltage-correction scenarios',fontsize=16,y=.994)
fig.legend(handles=[Line2D([],[],color='black',label='Observed mean (gray: repeats)'),Line2D([],[],color='#667085',ls=':',label='Fixed passive controls'),Line2D([],[],color='#d97706',label='Shared recruitment/decay time'),Line2D([],[],color='#087f8c',label='Separate recruitment/decay times')],loc='lower center',ncol=4,fontsize=10)
fig.tight_layout(rect=[0,.025,1,.963]);fig.savefig(ROOT/'validation/apl-recruitment.png',dpi=130);plt.close(fig)
