#!/usr/bin/env python3
"""Experiment-local four-arm recurrent kernel; production defaults unchanged.
    ...
    for k in range(nsteps):
        ...
        for cell in range(ncells):
            ...
            if available:
                if hybrid:
                    try:
                        result = hybrid_step(v[cell], old_s, old_h, .1, a, b, coefficient, NODES32, WEIGHTS32)
                    except Exception:
                        failure_code, failure_cell, failure_phase = 1, cell, 0
                        failed_before[0], failed_before[1], failed_before[2] = v[cell], old_s, old_h
                        break
                    v[cell], s[cell], h[cell] = result[0], result[1], result[2]
                    work[k, 1] += int(old_h > 0.)
                    work[k, 2] = max(work[k, 2], result[5])
                    solver[k, 0] = max(solver[k, 0], result[4])
                    solver[k, 1] = max(solver[k, 1], result[3])
                else:
