...
OUT = ROOT / "validation/inhibitory-recurrent-panel-combined-figure.png"
...
def main():
    ...
    for ax,cohort,title in zip(axs[0],["non_source","first_hop_non_source"],
        ...
        for a in arms:
            for si,seed in enumerate(seeds):
                ...
                ax.stairs(v,np.array(data["window_edges_ticks"])*.0001,color=colors[a],ls=styles[si],lw=1.4,alpha=.9)
        ...
        ax.set(title=title,xlabel="Simulated time (s)",ylabel="Window mean spikes/s/cell",xlim=(0,3))
    ...
    for ax,cohort,title in zip(axs[1],["first_hop_non_source","non_source"],
        ...
        ax.set(title=title,ylabel="Pulse minus matched constant (Hz/cell)",xlim=(-.5,3.5))
    ...
    fig.text(.085,.074,"Top: seven fixed window means, not instantaneous traces. Bottom: every seed shown; no confidence intervals.",fontsize=10,color="#444444")
    fig.text(.085,.045,"No-input and source-output-blocked controls have zero nonsource spikes in every arm and seed.\nPersistence is observed only through 3 s; numerical bounds and local contrast do not validate physiology or behavior.",fontsize=10,color="#444444",linespacing=1.6)
    ...
    MANIFEST.write_text(json.dumps(dict(source=before,script=record(Path(__file__)),output=record(OUT),
        plot="All four arms and three individual paired RNG seeds. Constant-input window means and predefined matched pulse differences; no biological confidence intervals or newly fitted targets."),indent=2)+"\n")
