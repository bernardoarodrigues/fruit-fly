...
class EdgeDeliveryInterventionNetwork(original.FactorialNetwork):
    ...
    def replace_selected_indices(self, indices):
        """Replace observation columns only, normally after checkpoint restore.

        Caller supplies explicit ordered unique indices; no cells are inferred.
        Input membership, all neural state and the pending queue are untouched.
        Save the original checkpoint selection before expanding observations if
        exact archived-checkpoint identity is required. Subsequent checkpoint()
        records the new selection, and default selected-event logging follows it.
        """
        if self.failed:
            raise RuntimeError("Cannot relabel observations of retained partial failure state")
        selected = self._indices(indices).copy()
        column = np.full(self.n_neurons, -1, np.int32)
        column[selected] = np.arange(len(selected), dtype=np.int32)
        # All validation/allocation precedes the two observation-only assignments.
        self.selected_indices, self._selected_column = selected, column
        return self
