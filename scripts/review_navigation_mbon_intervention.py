...
from review_navigation_ladder_mbon_inputs import incoming_edges, expanded_edges, TARGETS, WINDOWS, DECAY

EXPERIMENT = ROOT / 'validation/navigation-mbon-intervention-plan.json'
PREFIX = ROOT / 'validation/navigation-mbon-intervention-independent-review'
...
EXPERIMENT_SHA = 'dbe66e07151e24bbbaafb3d23f52f64a8aa2c74420f0ad75dc732e3dfb8710bc'
RESULT_SHA = 'e76a210217b204d711de4afbf839127356dee72264e85b1d8e4f0db716525d29'
...
class ConditionalInputs:
    ...
    def __init__(self, graph, target_indices, n, p, h, tick, tail_t, tail_i, masked_rows, window):
        ...
        self.suppressed = np.zeros_like(self.arrivals)
    ...
    def consume(self, ticks, indices, end):
        ...
        suppressed = self.mask[edges] & (event_t >= self.window[0]) & (event_t < self.window[1])
        events = np.column_stack((event_t, event_source, self.g['rows'][edges], self.targets[columns],
                                  np.where(suppressed, 3, 0))).astype(np.int64)
        bins = np.searchsorted(WINDOWS, event_t, side='right') - 1
        np.add.at(self.arrivals, (bins, edges), 1)
        np.add.at(self.suppressed, (bins[suppressed], edges[suppressed]), 1)
        ...
        for j in range(end - start):
            ...
            w, col, accept = weight[sl], columns[sl], ~suppressed[sl]
            positive, negative = accept & (w >= 0), accept & (w < 0)
        ...
        self.tick = end
        return dict(p=p, h=h, events=events, suppressed_events=events[suppressed],
...
def fixture():
    ...
    reduction = ConditionalInputs(g, targets, 4, initial_p, initial_h, 0,
                                  np.empty(0, np.int64), np.empty(0, np.int32), np.array([0, 2]), [19, 68])
    pieces, events = [], []
    ...
    for tick in range(100):
        ...
        for emitted, source in zip(ticks, indices):
            ...
            for edge in range(g['source_ptr'][source], g['source_ptr'][source + 1]):
                suppressed = edge in (0, 2) and 19 <= tick < 68
                expected_events.append([tick, int(source), edge, int(targets[g['columns'][edge]]), 3 if suppressed else 0])
                if not suppressed:
    ...
    assert reduction.suppressed.sum() == sum(e[-1] == 3 for e in expected_events)
    return dict(passed=True, checks=7, scope='Manufactured supplied events only: float32 promotion, ordered positive/negative/zero additions, chunk/18tick delay and half-open suppression boundaries')
...
def prepare():
    ...
    paths = [Path(__file__), ROOT / 'scripts/review_inhibitory_recurrent_panel_trial.py',
    ...
    plan = dict(schema=1, created_utc=datetime.now(timezone.utc).isoformat(), inputs=pins,
        ...
        methods=['Independent immutable archive decoder verifies descriptors, hashes, dtype/shape and packed spike restoration.',
                 ...
                 'Join every source emission to original CSR incoming edges for10 targets at emission+18; suppress exactly the8236 declared positive edges only on delivery ticks[5000,15000). Compare every ordered logged arrival and suppressed sidecar record.',
                 'Reconstruct10 target p/h in original float64 operation order with float32 stored weights, decay before arrival, H1 receives throughout refractory/reset; compare all25001 trace rows bitwise and all6 checkpoints.',
                 'Reconstruct global edge counts by source out-degree and sign, without propagating state; verify no blocked/unavailable counts and the accepted/suppressed partition.',
                 'Verify58 selected-state continuity, availability, threshold/firing/reset/direct-input phase identities; numerical interval solutions are independently reviewed elsewhere.'],
        outputs=dict(result=str(RESULT.relative_to(ROOT)), arrays=str(ARRAYS.relative_to(ROOT))),
        array_schema='Per trial: full7xN window_counts, MBON p/h25001x10, incoming edge arrival/accepted/suppressed7xE, and exact postbranch MBON tick/index records. Common target/edge identities.',
        ...
        limits=['Conditional synaptic-state reconstruction uses the newly emitted recurrent spike history; no full neural/voltage re-integration occurs.',
                ...
                'The pathway was selected using the same prior six controls; the intervention does not establish unique causation, biological KC ablation, calibrated depression or navigation.'])
    write(PLAN, plan)
...
def run():
    ...
    try:
        ...
        ck('fixed_contract', experiment['start_tick'] == 5000 and experiment['end_tick'] == 30000
           ...
           and experiment['mbon_indices'] == TARGETS.tolist() and experiment['window_edges_ticks'] == WINDOWS.tolist()
        ...
        with np.load(ROOT / 'validation/navigation-ladder-mbon-input-arrays.npz', allow_pickle=False) as z:
            mask_rows = z['proposed_intervention_edge_indices'].copy()
            same('incoming_original_edge_inventory', graph['rows'], z['edge_indices'])
            same('incoming_original_weights', graph['weights'], z['edge_weights'])
        same('declared_intervention_edges', mask_rows, np.array(experiment['suppressed_edge_indices'], np.int64))
        ck('positive_target_edges', len(mask_rows) == 8236 and np.all(graph['weights'][np.searchsorted(graph['rows'], mask_rows)] > 0))
        ...
        mbon_cols = np.searchsorted(selected, TARGETS)
        same('expanded_selection', selected, np.unique(np.r_[selection['selected'], TARGETS]).astype(np.int32))
        ck('no_direct_MBON_input', not np.intersect1d(inputs, TARGETS).size)
        ...
        arrays.update(target_indices=TARGETS, selected_indices=selected, edge_indices=graph['rows'],
                      ...
                      suppressed_edge_indices=mask_rows, window_edges_ticks=WINDOWS)
        for spec in experiment['trials']:
            ...
            reduction = ConditionalInputs(graph, TARGETS, n, initial['s'][TARGETS], initial['h'][TARGETS],
                                          5000, tail_t, tail_i, mask_rows, [5000, 15000])
            p_trace, h_trace = np.full((25001, 10), np.nan), np.full((25001, 10), np.nan)
            ...
            own_t, own_i = [], []
            totals = dict(spikes_after_branch=0, suppressed_deliveries=0, candidate_events=0, applied_events=0,
            ...
            def checkpoint(tick, cp=None):
                ...
                same('checkpoint_edge_mask', ins['suppressed_edge_indices'], mask_rows, [ordinal, tick])
                ck('checkpoint_intervention_spec', ins['delivery_window_ticks'] == [5000, 15000] and ins['enabled']
                   and ins['graph_sha256'] == experiment['graph_sha256'] and ins['dt_ms'] == .1, [ordinal, tick])
            ...
            for chunk in range(100, 600):
                ...
                ck('selected_state_domain', pre.shape == (50, 58) and np.isfinite(pre).all()
                   and all(out['selected_' + k].shape == (51, 58) and out['selected_' + k].dtype == np.float64
                ...
                for state, trace, saved_state in (('p', p_trace, 's'), ('h', h_trace, 'h')):
                    same('complete_MBON_conditional_' + state, out['selected_' + saved_state][:, mbon_cols], reduced[state], [ordinal, chunk])
                    trace[start - 5000:end - 5000 + 1] = reduced[state]
                result['context']['conditional_trace_end_tick'] = end
                same('every_ordered_target_arrival', out['selected_events'], reduced['events'], [ordinal, chunk])
                ck('event_target_schema', out['schema']['event_graph_indices'] == TARGETS.tolist(), [ordinal, chunk])
                side = out['edge_intervention']
                same('every_suppressed_edge_tick_source_target_disposition', side['events'], reduced['suppressed_events'], [ordinal, chunk])
                suppression_counts = np.zeros((50, 3), np.int64)
                np.add.at(suppression_counts[:, 2], reduced['suppressed_events'][:, 0] - start, 1)
                same('suppression_counts', side['counts'], suppression_counts, [ordinal, chunk])
                ck('suppression_sidecar_clock', side['start_tick'] == start and side['end_tick'] == end
                   and side['requested_ticks'] == 50 and side['partial'] is None, [ordinal, chunk])
                ...
                same('all_graph_signed_edge_acceptance', edge_counts[:, 1], visited - suppression_counts, [ordinal, chunk])
                ...
                totals['suppressed_deliveries'] += len(side['events'])
                ...
                totals['accepted_target_arrivals'] += len(reduced['events']) - len(side['events'])
            ...
            ck('reported_trial_totals', totals['spikes_after_branch'] == report['spikes_after_branch']
               and totals['suppressed_deliveries'] == report['suppressed_deliveries'], ordinal)
            arrays[name + 'edge_arrivals'] = reduction.arrivals
            arrays[name + 'edge_suppressed'] = reduction.suppressed
            arrays[name + 'edge_accepted'] = reduction.arrivals - reduction.suppressed
            arrays[name + 'MBON_spike_ticks'] = np.concatenate(own_t)
            arrays[name + 'MBON_spike_indices'] = np.concatenate(own_i)
            result['completed_trials'].append(dict(spec=spec, passed=True, checkpoint_ticks=experiment['checkpoint_ticks'],
                ...
                complete_MBON_p_h_bitwise=True, MBON_window_counts=counts[:, TARGETS].tolist(),
    ...
    result.update(passed=result['passed'] and not audit.failures and not result['errors'],
        ...
        numerical_reference_review='Delegated separately; this receipt checks saved state/event/count semantics only')
    write(RESULT, result)
