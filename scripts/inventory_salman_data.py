...
def main():
    ...
    parser.add_argument("--resume", type=Path, help="Reuse exactly hashed metadata from a retained partial inventory")
    ...
    if args.resume:
        ...
        for record in prior["metadata_requests"]:
            raw = (ROOT / record["path"]).read_bytes()
            if len(raw) != record["bytes"] or hashlib.sha256(raw).hexdigest() != record["sha256"]:
                raise ValueError("Retained metadata hash mismatch")
            cache[record["url"]] = json.loads(raw)
            records.append(record)
        resume = {"path": str(args.resume), "sha256": hashlib.sha256(args.resume.read_bytes()).hexdigest(),
                  "reused_metadata_responses": len(records), "prior_started_utc": prior["started_utc"]}
    ...
    try:
        while queue:
            ...
            license_id = (node["relationships"].get("license", {}).get("data") or {}).get("id")
