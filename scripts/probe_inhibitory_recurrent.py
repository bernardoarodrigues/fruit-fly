...
PLAN = ROOT / 'validation/inhibitory-recurrent-performance-plan.json'
OUT = ROOT / 'validation/inhibitory-recurrent-performance'
...
def freeze():
    ...
    paths = [Path(__file__), ROOT/'scripts/inhibitory_recurrent_kernel.py',
...
def scalar_tile(grid, repetitions):
    ...
    for repeat in range(repetitions):
        for i in range(len(grid)):
            ...
            for j in range(7):results[i,j]=result[j]
            checksum+=result[0]
